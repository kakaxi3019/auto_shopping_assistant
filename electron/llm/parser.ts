...
用户的历史订单如下（每行格式：序号. [ID:数字] [平台:名称] [价格:金额元] [购买时间:日期] [已排除] 商品名称）：
{orderHistory}
  ...
  private buildSystemPrompt(): string {
    ...
    const orderLines = orders
      ...
      .map((o, i) => {
        ...
        return `${i + 1}. [ID:${o.id}] [平台:${platLabel}]${priceStr}${dateStr}${excluded} ${o.productName}`
      })
