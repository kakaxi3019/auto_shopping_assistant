...
【重要】每个商品的唯一标识就是当前给出的真实历史订单列表中商品前面的序号（如 1、2、3...）。在输出的 orderRef 和 matchedOrders 中，你必须根据当前真实的订单列表选择匹配的序号。
【特别警告】严禁死记硬背或照抄示例中的输出！示例中的输入指令和序号仅用于格式演示。在实际处理中，即使用户的输入指令与示例一模一样，你也必须根据当前真实的订单列表进行匹配，绝对不能使用真实列表中不存在的序号，也决不能强行套用示例中的序号！
...
历史订单：
1. [平台:淘宝] 户外防风运动水壶儿童用
2. [平台:淘宝] Columbia防风外套男士
3. [平台:淘宝] 探路者户外冲锋衣防风衣外套
输出：{"items":[{"name":"防风外套","quantity":1,"orderRef":2,"platform":"taobao","matchedOrders":[{"orderRef":2,"confidence":95},{"orderRef":3,"confidence":81}]}]}
注意：序号1虽然有"防风"字样，但商品本体是水壶不是外套，故不匹配。
...
export class LlmParser {
  ...
  private getApiKey(): string {
    ...
    if (!key || key.trim() === '') throw new Error('请先在设置中配置 API Key')
    return key
  }
  ...
  private async verifyOpenAI(): Promise<{ success: boolean; error?: string }> {
    ...
    // 仅当用户已经在数据库中显式保存过模型名称时，才进行强校验。
    // 首次配置未设置模型时（即数据库中 model 为空，使用的是 gpt-4o-mini 兜底），
    // 只要 /models 能正常返回模型列表，即证明 Key 和 Base URL 有效，直接放行通过。
    const hasConfiguredModel = !!this.db.getSetting(this.getSettingKey('model'))
    if (hasConfiguredModel && models && !models.some(m => m.id === model)) {
  ...
  async parse(instruction: string): Promise<ParsedShoppingItem[]> {
    ...
    const orders = this.getMergedOrdersForLlm().filter(o => o.productName)
  ...
  private buildSystemPrompt(): string {
    const orders = this.getMergedOrdersForLlm()
  ...
  private getMergedOrdersForLlm(): any[] {
    const taobaoOrders = this.db.getOrders('taobao', 100, 0)
    const jdOrders = this.db.getOrders('jd', 100, 0)
    const pddOrders = this.db.getOrders('pdd', 100, 0)
    
    const merged = [...taobaoOrders, ...jdOrders, ...pddOrders]
  ...
  private async parseWithOpenAI(instruction: string, systemPrompt: string): Promise<ParsedShoppingItem[]> {
    ...
    try {
      ...
      if (
        errMsg.includes('400') ||
        errMsg.includes('Param') ||
        errMsg.includes('Incorrect') ||
        errMsg.includes('format') ||
        errMsg.includes('response_format') ||
        errMsg.includes('parameter')
      ) {
        console.warn(`[LlmParser] OpenAI completion failed with format error, retrying without response_format: ${errMsg}`)
        ...
        if (!fallbackContent) throw new Error('LLM 返回为空')
        return this.parseContent(fallbackContent)
      }
      throw e
    }
  }
