...
用户的历史订单如下（每行格式：序号. [ID:数字] [平台:名称] [已排除] 商品名称）：
{orderHistory}

核心原则：用户说"买XX"，绝大多数情况是想再买之前买过的同类商品，而不是买全新的东西。所以你必须优先从历史订单中寻找匹配。

匹配规则（按优先级排列）：
1. 商品本体匹配：用户要买的东西必须与历史订单商品是同一种商品。判断标准是"商品本身是什么"，而不是"商品名中是否包含用户说的词"。例如用户说"买篮球"，只有商品本身是篮球的才匹配，篮球鞋、篮球护具、篮球裤虽然商品名含"篮球"但商品本体不是篮球，不应匹配
...
8. 置信度低于 50 的商品绝对不要放入 matchedOrders，只放入你有较高把握认为是用户想买的商品
...
3. [ID:12] [平台:京东] [已排除] 安踏静音篮球篮筐
4. [ID:15] [平台:淘宝] 专柜正品adidas新款男子运动鞋
5. [ID:20] [平台:淘宝] 伊利纯牛奶250ml*16
6. [ID:25] [平台:淘宝] 蒙牛纯牛奶250ml*24
7. [ID:30] [平台:淘宝] 安踏篮球鞋男款
8. [ID:40] [平台:淘宝] 斯伯丁篮球7号
9. [ID:45] [平台:淘宝] LP768篮球护踝运动防护
...
输入："买一个篮球"
输出：{"items":[{"name":"篮球","quantity":1,"orderRef":40,"platform":"taobao","matchedOrders":[{"orderRef":40,"confidence":90}]}]}
注意：只匹配商品本体是篮球的(ID:40)。ID:12已排除，ID:30篮球鞋不是篮球，ID:45护踝不是篮球，都不匹配

输入："买一双篮球鞋"
输出：{"items":[{"name":"篮球鞋","quantity":1,"orderRef":30,"platform":"taobao","matchedOrders":[{"orderRef":30,"confidence":95}]}]}
...
输入："买一双拖鞋"
历史订单：
1. [ID:5] [平台:淘宝] 上市品牌】可孚医用耳温枪儿童用
2. [ID:8] [平台:淘宝] Babycare儿童防水棉拖鞋
3. [ID:12] [平台:淘宝] 儿童亚麻拖鞋男女童宝宝棉麻布拖
输出：{"items":[{"name":"拖鞋","quantity":1,"orderRef":8,"platform":"taobao","matchedOrders":[{"orderRef":8,"confidence":95},{"orderRef":12,"confidence":85}]}]}
注意：ID:5耳温枪不匹配，虽然都是"儿童"用品，但商品本体是耳温枪不是拖鞋，且"拖鞋"不出现在商品名中
  ...
  private buildSystemPrompt(): string {
    ...
    const orderLines = orders
      ...
      .map((o, i) => {
        ...
        return `${i + 1}. [ID:${o.id}] [平台:${platLabel}]${excluded} ${o.productName}`
      })
